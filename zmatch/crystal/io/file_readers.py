...
from zmatch.crystal.phase import Phase
...
class OxfordBinaryLoader: #EBSDDataLoader
    ...
    def load_oxford_cpr(self, file_name: str) -> None:
        """
        ...
        cpr_file_name = "{}.cpr".format(file_name)
        cpr_file_path = pathlib.Path(file_name) / pathlib.Path(cpr_file_name)
        if not cpr_file_path.is_file():
            raise FileNotFoundError("Cannot open file {}".format(cpr_file_path))
        ...
        with open(str(cpr_file_path), 'r') as cprFile:
            while True:
        ...
        self.col_num = int(metadata['Job']['xCells'])
        self.row_num = int(metadata['Job']['yCells'])
        self.step_size = float(metadata['Job']['GridDistX'])
        ...
        for i in range(num_phases):
            ...
            new_phase.load_base_information(
            ...
            self.phase_information.append(new_phase)
        ...
        self.data_format = np.dtype(dataFormat)
    ...
    def load_oxford_crc(self, file_name: str, file_dir: str = "") -> None:
        ...
        """
        crc_file_name = "{}.crc".format(file_name)
        crc_file_path = pathlib.Path(file_dir) / pathlib.Path(crc_file_name)
        if not crc_file_path.is_file():
            raise FileNotFoundError("Cannot open file {}".format(crc_file_path))

        # load binary data from file
        binData = np.fromfile(str(crc_file_path), self.data_format, count=-1)

        self.band_contrast_map = np.reshape(binData['BC'], (self.row_num, self.col_num))
        self.band_slope_map = np.reshape(binData['BS'], (self.row_num, self.col_num))
        self.mean_angular_deviation_map = np.reshape(binData['MAD'], (self.row_num, self.col_num))
        self.phase_map = np.reshape(binData['phase'], (self.row_num, self.col_num))
        euler_angle_map = np.reshape(binData[['ph1', 'phi', 'ph2']], (self.row_num, self.col_num))

        self.euler_angle_map = np.array(euler_angle_map.tolist()).transpose((2, 0, 1))
        ...
        #         [(field[4:], np.reshape(binData[field], (self.row_num, self.col_num))) for field in EDXFields])
