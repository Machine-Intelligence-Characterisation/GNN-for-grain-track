...
def calc_quat_by_symmetry_quat(quat_coef, sym_quat):
    """

    Notes
    -----
    It is the Hamilton product of the two quaternions. For these symmetrical operations, the order for two quats
    does not matter. But the order for the quaternion and the vector does matter.

    """
    target_quat = np.zeros(quat_coef.shape, dtype=float)

    target_quat[0] = (sym_quat[0] * quat_coef[0] - sym_quat[1] * quat_coef[1] -
                      sym_quat[2] * quat_coef[2] - sym_quat[3] * quat_coef[3])
    target_quat[1] = (sym_quat[0] * quat_coef[1] + sym_quat[1] * quat_coef[0] +
                      sym_quat[2] * quat_coef[3] - sym_quat[3] * quat_coef[2])
    target_quat[2] = (sym_quat[0] * quat_coef[2] - sym_quat[1] * quat_coef[3] +
                      sym_quat[2] * quat_coef[0] + sym_quat[3] * quat_coef[1])
    target_quat[3] = (sym_quat[0] * quat_coef[3] + sym_quat[1] * quat_coef[2] -
                      sym_quat[2] * quat_coef[1] + sym_quat[3] * quat_coef[0])

    target_quat[target_quat[0] < 0] = -target_quat[target_quat[0] < 0]

    return target_quat


def calc_symmetry_equivalents(quat_coef, sym_quat):
    """Calculate all symmetry equivalent quaternions for a given quaternion."""
    quat_coef = np.array(quat_coef, dtype=float)
    sym_equivs = np.zeros((len(sym_quat), 4), dtype=float)
    for i, sym in enumerate(sym_quat):
        sym_equivs[i] = calc_quat_by_symmetry_quat(quat_coef, sym)

    return sym_equivs


def calc_misorientation(q1, q2, sym_quat=None):
    """Calculate the misorientation angle between two quaternions."""
    if sym_quat is not None:
        q2_quat_with_sym = np.zeros((len(sym_quat), 4), dtype=float)
        for i, sym in enumerate(sym_quat):
            q2_quat_with_sym[i] = calc_quat_by_symmetry_quat(q2, sym)

        misorientation = q1 * q2_quat_with_sym
...
def quat_from_euler_angle(euler_array):
    ph1, phi, ph2 = euler_array[0], euler_array[1], euler_array[2]
    quat_coef = np.array([
...
def calc_rotate_matrix(quat_coef) -> np.ndarray:
    ...
    ----------
        ...
        based algorithm to be used for crystal structure simulations',
    ...
    qbar = quat_coef[0]**2 - quat_coef[1]**2 - quat_coef[2]**2 - quat_coef[3]**2

    rotate_matrix[0, 0] = qbar + 2 * quat_coef[1]**2
    rotate_matrix[0, 1] = 2 * (quat_coef[1] * quat_coef[2] - quat_coef[0] * quat_coef[3])
    rotate_matrix[0, 2] = 2 * (quat_coef[1] * quat_coef[3] + quat_coef[0] * quat_coef[2])

    rotate_matrix[1, 0] = 2 * (quat_coef[1] * quat_coef[2] + quat_coef[0] * quat_coef[3])
    rotate_matrix[1, 1] = qbar + 2 * quat_coef[2]**2
    rotate_matrix[1, 2] = 2 * (quat_coef[2] * quat_coef[3] - quat_coef[0] * quat_coef[1])

    rotate_matrix[2, 0] = 2 * (quat_coef[1] * quat_coef[3] - quat_coef[0] * quat_coef[2])
    rotate_matrix[2, 1] = 2 * (quat_coef[2] * quat_coef[3] + quat_coef[0] * quat_coef[1])
    rotate_matrix[2, 2] = qbar + 2 * quat_coef[3]**2
...
def calc_vector_after_rotation(quat_coef, vector) -> np.ndarray:
    """Calculate the vector after rotation by the quaternion."""

    rotate_matrix = calc_rotate_matrix(quat_coef)
    return rotate_matrix @ vector


def rotate_vec_by_quat(q, v):
    vq = np.array([0.0, *v])
    rq = hamilton_product(hamilton_product(q, vq), conjugate(q))
    return rq[1:]


def calc_euler_angle_from_quat(quat_coef) -> np.ndarray:
...
def norm(quat_coef) -> float:
    ...
    """
    return np.sqrt(np.dot(quat_coef, quat_coef))


def normalise(quat_coef):
    ...
    """
    quat_coef /= norm(quat_coef)

    return quat_coef


# also the inverse if this is a unit quaternion
def conjugate(quat_coef):
    ...
    return np.array([quat_coef[0], -quat_coef[1], -quat_coef[2], -quat_coef[3]], dtype=float)
...
def stereographic_projection(v):
    v = np.array(v)
    if v[2] <= 0:
        return None  # Not in upper hemisphere
    return v[0] / (1 + v[2]), v[1] / (1 + v[2])


def calc_ipf_direction(quat_coef, sym_quat, sym_group='cubic', direction=np.array([0, 0, 1])):
    quat_with_sym = np.zeros((len(sym_quat), 4), dtype=float)
    for i, sym in enumerate(sym_quat):
        quat_with_sym[i] = calc_quat_by_symmetry_quat(quat_coef, sym)

    # vector = rotate_vec_by_quat(quat_coef, direction)
    # vector /= np.linalg.norm(vector)

    all_vectors = []

    for q in quat_with_sym:
        v_rotated = rotate_vec_by_quat(q, direction)
        v_rotated /= np.linalg.norm(v_rotated)

        # v_rotated = rotate_vec_by_quat(conjugate(q), direction)

        # v_rotated = calc_vector_after_rotation(conjugate(q), direction)
        # v_rotated = v_rotated / np.sqrt(np.dot(v_rotated, v_rotated))

        if v_rotated[2] < 0:
            v_rotated = -v_rotated

        all_vectors.append(v_rotated)
    ...
    for v in all_vectors:
        if any(v < 0):
            continue
        if sym_group == 'cubic':
            alpha = np.arccos(v[2])  # inclination
            beta = np.arctan2(v[1], v[0])  # azimuth

            if beta < 0:
                beta += 2 * np.pi

            delta = 1e-8
            if not (beta >= -delta) & (beta <= np.pi / 4 + delta):
                continue
...
class Quat:
    ...
    def calc_quat_by_symmetry_quat(quat_coef, sym_quat):
        ...
        target_quat[0] = (quat_coef[0] * sym_quat.quat_coef[0] - quat_coef[1] * sym_quat.quat_coef[1] -
                            quat_coef[2] * sym_quat.quat_coef[2] - quat_coef[3] * sym_quat.quat_coef[3])
        target_quat[1] = (quat_coef[0] * sym_quat.quat_coef[1] + quat_coef[1] * sym_quat.quat_coef[0] -
                            quat_coef[2] * sym_quat.quat_coef[3] + quat_coef[3] * sym_quat.quat_coef[2])
        target_quat[2] = (quat_coef[0] * sym_quat.quat_coef[2] + quat_coef[2] * sym_quat.quat_coef[0] -
                            quat_coef[3] * sym_quat.quat_coef[1] + quat_coef[1] * sym_quat.quat_coef[3])
        target_quat[3] = (quat_coef[0] * sym_quat.quat_coef[3] + quat_coef[3] * sym_quat.quat_coef[0] -
                            quat_coef[1] * sym_quat.quat_coef[2] + quat_coef[2] * sym_quat.quat_coef[1])
    ...
    def calc_misorientation(q1, q2, sym_quat=None):
        """Calculate the misorientation angle between two quaternions."""
        if sym_quat is not None:
            q2_quat_with_sym = Quat.calc_quat_by_symmetry_quat(q2.quat_coef, sym_quat)
    ...
    @staticmethod
    def quat_from_euler_angle(ph1, phi, ph2):
        quat_coef = np.array([
            np.cos(phi / 2.0) * np.cos((ph1 + ph2) / 2.0),
            -np.sin(phi / 2.0) * np.cos((ph1 - ph2) / 2.0),
            -np.sin(phi / 2.0) * np.sin((ph1 - ph2) / 2.0),
...
# Static methods
    ...
    @staticmethod
    def create_quats_from_euler(eulerArray: np.ndarray) -> np.ndarray:
